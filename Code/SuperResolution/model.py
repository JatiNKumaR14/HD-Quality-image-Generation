...
class SRDense(object):
    ...
    def train(self, config):
        ...
        nx, ny,lr = input_setup(config)

        data_dir = checkpoint_dir(config)


        input_, label_ = read_data(data_dir)
        print(input_.shape," ->>>>>> input shape",label_.shape," ->>>>>> label shape")
        ...
        # Test
            ...
            for i in range(len(input_)):
                if self.test_img is "":
                    imsave(res[i], config.result_dir + '/result_'+str(i)+'.png', config)
                else:
                    string = self.test_img.split(".")
                    print(string)
                    print(res[i].shape)

                    imsave(res[i], '/content/'+config.result_dir + '/' + string[0] + '.png', config)
